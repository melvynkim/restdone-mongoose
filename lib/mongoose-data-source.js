...
class MongooseDataSource {
  ...
  find(options) {
    const { fields, filter, q, qFields, queryPipe, restrictFields = true, scope, sort } = options;
    const resolved = this._resolveAssociations(fields, scope);
  ...
  findOne(options) {
    const { fields, filter, queryPipe, restrictFields = true, scope } = options;
    const resolved = this._resolveAssociations(fields, scope);
    const { populationQuery } = resolved;
    this._normalizeFilter(filter, this.ModelClass);
  ...
  count(options) {
    ...
    return this.ModelClass.countDocuments(filter).exec();
  }
  ...
  _resolveAssociations(fields, scope) {
    ...
    function recursiveResolve(ModelClass, obj) {
      ...
      if (obj.filter) {
        query.match = _.isFunction(obj.filter) ? obj.filter(scope) : obj.filter;
      }
      ...
      _.forEach(obj.fields, (field) => {
        if (_.isString(field)) {
          ...
          const ref = (branch[0] && branch[0].ref) || branch.ref || branch.options.ref;
